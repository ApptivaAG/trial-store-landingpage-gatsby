    ...
    <div className="article" key={path}>
      <div className="container">
        ...
        <div className="content">
          <p>{description}</p>
          {fixed ? <Img fixed={fixed} style={{ display: 'block', margin: '0 auto' }} /> : ''}
        </div>
        <div
          className="content"
...
const Button = ({ path, label, primary }) => (
  <Link className="readmore" to={path}>
    <span className={`btn btn-outline-primary btn-block ${primary ? 'btn-outline-primary' : 'btn-outline-secondary'}`}>
      {label}
    </span>
  </Link>
)
